...
class PHLOPVideoDataset(Dataset):
    ...
    def _get_taxonomy_summary(self, metadata):
        obj_events = {}
        for frame in metadata.get("frames", []):
            for obj_id, obj_data in frame.get("objects", {}).items():
                obj_events.setdefault(obj_id, set())
                for entry in obj_data.get("taxonomy", []):
                    for label in entry.get("labels", []):
                        obj_events[obj_id].add(
                            f"{entry['subcategory']} ({label})"
                        )

        return "\n".join(
            f"Object {obj_id} events: {', '.join(sorted(labels))}"
            for obj_id, labels in obj_events.items()
        )
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "taxonomy_summary": self._get_taxonomy_summary(metadata),
