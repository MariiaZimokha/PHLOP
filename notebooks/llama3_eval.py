"""
VideoLLaMA3-specific PHLOP eval: zero-shot, fine-tuning, test comparison.
...
Notebook usage:
    ...
    from llama3_eval import finetune_single_config_llama3
    ...
    for cfg_name in FINE_TUNE_CONFIGS:
        for use_physics in [False, True]:
            ckpt = finetune_single_config_llama3(
                splits,
                cfg_name=cfg_name,
...
LLAMA3_MODEL_ID = "DAMO-NLP-SG/VideoLLaMA3-2B"
LLAMA3_MODEL_REVISION = "main"
NUM_VIDEO_FRAMES = 16
...
def _build_conversation(
    ...
    prompt: str,
    fps: int = VIDEO_FPS,
    max_frames: int = NUM_VIDEO_FRAMES,
    answer: str = None,
) -> list[dict]:
    """Build a VideoLLaMA3 conversation list."""
    conv = [
...
def collect_predictions_llama3(
    model,
    processor,
    dataset,
    max_samples=None,
    ...
    tmp_dir = tempfile.mkdtemp(prefix="llama3_batches_")
    ...
    def _prepare_item(idx):
        ...
        if not video_path or not os.path.exists(video_path):
            return idx, None
        try:
            conv = _build_conversation(video_path, item["prompt"], fps=fps, max_frames=max_frames)
            inputs = processor(conversation=conv, return_tensors="pt")
        except Exception:
            return idx, None
    ...
    with ThreadPoolExecutor(max_workers=prefetch_workers) as pool:
        ...
        for _ in tqdm(range(n), desc="Collecting predictions", mininterval=5):
            ...
            if i < 10 or i % 100 == 0:
                qa_q = (item.get("qa_entry") or {}).get("question", "")[:80]
                print(f"  [{i}] Q: {qa_q}")
                print(f"       raw: {raw_pred[:120]!r}")
                print(f"       pred: {pred!r}")
                print(f"       true: {(item.get('qa_entry') or {}).get('answer', '')!r}")
...
class LLaMA3DataCollator:
    """Collator that builds VideoLLaMA3 conversations with video for training.
    Supports batch_size >= 1."""

    def __init__(self, processor, fps: int = VIDEO_FPS, max_frames: int = NUM_VIDEO_FRAMES):
    ...
    def __call__(self, batch):
        ...
        all_inputs = [self._process_single(item) for item in batch]
        ...
        for inp in all_inputs:
            ...
            if pad_len > 0:
                padded_ids.append(torch.cat([
                    torch.full((1, pad_len), pad_id, dtype=inp["input_ids"].dtype),
                    inp["input_ids"],
                ], dim=1))
                padded_attn.append(torch.cat([
                    torch.zeros(1, pad_len, dtype=inp["attention_mask"].dtype),
                    inp["attention_mask"],
                ], dim=1))
                padded_labels.append(torch.cat([
                    torch.full((1, pad_len), -100, dtype=inp["labels"].dtype),
                    inp["labels"],
                ], dim=1))
            else:
                padded_ids.append(inp["input_ids"])
                padded_attn.append(inp["attention_mask"])
                padded_labels.append(inp["labels"])
        ...
        pv_list = [inp["pixel_values"] for inp in all_inputs if "pixel_values" in inp]
        if pv_list:
            result["pixel_values"] = torch.cat(pv_list, dim=0)

        for key in all_inputs[0]:
            if key not in result:
                vals = [inp[key] for inp in all_inputs if key in inp]
    ...
    def _process_single(self, item):
        try:
            ...
            inputs = self.processor(conversation=conv, return_tensors="pt")
            if "pixel_values" in inputs:
                inputs["pixel_values"] = inputs["pixel_values"].to(torch.bfloat16)
            inputs["labels"] = inputs["input_ids"].clone()
            return inputs
        except Exception as e:
            print(f"  Warning: skipping sample (video={item.get('video_path', '?')}): {e}")
            conv = _build_conversation(
                item["video_path"], item["prompt"],
                fps=self.fps, max_frames=0,
                answer=item["target"],
            )
            text = self.processor.tokenizer.apply_chat_template(
                [{"role": "user", "content": item.get("prompt", "")},
                 {"role": "assistant", "content": item.get("target", "")}],
                tokenize=False,
            )
            tokens = self.processor.tokenizer(text, return_tensors="pt")
            tokens["labels"] = torch.full_like(tokens["input_ids"], -100)
            return tokens


class LLaMA3Trainer(Trainer):
...
def run_zero_shot_llama3(
    ...
    if processor is None:
        processor = AutoProcessor.from_pretrained(
            LLAMA3_MODEL_ID, trust_remote_code=True, revision=LLAMA3_MODEL_REVISION,
        )
    if model is None:
    ...
    for split_name in available:
        ...
        results = collect_predictions_llama3(
            model, processor, val_ds,
            max_samples=max_samples, device=device,
            fps=fps, max_frames=num_frames,
        ...
        if results_dir:
            pred_path = os.path.join(
                results_dir,
                f"llama3_zero_shot_predictions_{split_name}_{camera_mode}_{physics_tag}.json",
            )
...
def finetune_single_config_llama3(
    splits: PHLOPSplits,
    cfg_name: str,
    output_dir: str = "./llama3_physics_full",
    max_steps: int = -1,
    num_epochs: int = 1,
    camera_mode: str = "static",
    use_physics: bool = False,
    ...
    """Fine-tune VideoLLaMA3 on a single difficulty config. Returns checkpoint dir or None.
    ...
    data_collator = LLaMA3DataCollator(processor, fps=fps, max_frames=num_frames)
    ...
    ckpt_dir = os.path.join(output_dir, f"llama3_{cfg_name}_{physics_tag}")
    use_bf16 = (device != "mps")
    use_fp16 = (device == "mps")
...
def run_test_comparison_llama3(
    splits: PHLOPSplits,
        ...
        for split_name in available:
            ...
            if results_dir:
                out_path = os.path.join(
                    results_dir,
                    f"llama3_finetuned_predictions_{model_name}_{split_name}_{camera_mode}_{physics_tag}.json",
                )
