...
\title{\textbf{PHLOP}: \textbf{P}hysics-Grounded \textbf{H}ierarchical \textbf{L}abels for \textbf{O}bject
\textbf{P}roperties*\\
...
\begin{abstract}
Physical video--QA benchmarks often report high scores without separating visual dynamics from textual shortcuts. We introduce \textbf{PHLOP} (\textit{Physics-grounded Hierarchical Labels for Object Properties}), a controllable MuJoCo diagnostic that pairs rigid-body video with per-object, per-frame taxonomy labels and real-valued parameters (mass, friction, elasticity). Factor-disjoint train/val/test splits and difficulty-graded questions support curriculum-style probing of open video--language models. Zero-shot accuracy is near chance; LoRA fine-tuning raises scores substantially, and textual physics hints help unevenly across models. Critically, several templates remain solvable from the question text alone, so we frame PHLOP as a \emph{leak-auditable curriculum and cue-sensitivity diagnostic}: what a researcher learns is how models adapt under curricula and non-visual cues on a fully attributed physics substrate---not that high accuracy equals grounded Newtonian understanding. The open generator, labels, and question typology make shortcut solvability measurable rather than hidden.
\end{abstract}
...
\caption{
\textbf{Comparison of physical-reasoning benchmarks} (authors' coding of each system; judgments are contestable).
Capabilities: \textit{Counterfactual Dynamics} $=$ engine re-simulation under interventions (not verbal what-ifs);
\textit{Few-shot Material Reasoning} $=$ inferring material-linked properties without numeric parameters in the question stem;
\textit{Per-frame Taxonomy} $=$ object-level event labels in a formal hierarchy at every frame.
Bold marks denser support on that row, not overall superiority.
\cmark = fully supported, \warn = partially supported, \xmark = not supported.
}
...
Counterfactual Dynamics              & \warn  & \warn  & \cmark & \cmark & \warn  & \xmark & \xmark & \xmark \\
Explicit Physical Properties         & \cmark & \warn  & \warn  & \xmark & \warn  & \xmark & \xmark & \warn \\
Few-shot Material Reasoning          & \warn  & \warn  & \warn  & \xmark & \xmark & \xmark & \xmark & \xmark \\
\midrule
...
Distribution-based Material Sampling & \cmark & \warn  & \warn  & \xmark & \xmark & \xmark & \xmark & \xmark \\
\bottomrule
\end{tabular}
...
Physical reasoning---understanding how objects move, collide, and respond to forces---is central to human cognition and to applications in robotics and vision \cite{yi2020clevrer,tung2023physion++,battaglia2013simulation,piloto2022intuitive}.
Video--language models are increasingly evaluated on physics-flavored QA, yet high accuracy can reflect \emph{template memorization} or \emph{textual shortcuts} rather than grounded dynamics \cite{tung2023physion++,melnik2023benchmarks,hofherr2023neural}.
Benchmarks that cannot audit that gap leave readers unsure what their leaderboards measure.

Existing resources each leave part of this gap open.
CLEVRER~\cite{yi2020clevrer} pioneered causal video QA but abstracts events into symbolic labels without continuous physical parameters.
COMPHY~\cite{chen2022comphy} supports compositional interventions (including engine re-simulation) but simplifies friction and omits rotational dynamics.
Physion++~\cite{tung2023physion++} varies properties and scene diversity yet lacks per-frame, taxonomy-structured event labels for fine-grained diagnosis.
What is missing is a substrate that is simultaneously (i)~controllable and attributed (real-valued parameters + per-frame labels), (ii)~split for domain shift, and (iii)~explicit about which questions can be solved without video.

We introduce \textbf{PHLOP} (Physics-Grounded Hierarchical Labels for Object Properties) to fill that niche.
PHLOP is not offered as a claim that fine-tuned VLMs ``understand Newtonian physics.''
It is a \textbf{leak-auditable diagnostic}: a MuJoCo-generated video--QA suite where every answer is tied to simulation logs, every event label sits in a hierarchical taxonomy, and question templates can be partitioned into video-required vs.\ video-optional types (Section~\ref{sec:threats}).
A researcher using PHLOP can therefore ask---and, with a text-only ablation, quantify---how much of a model's score comes from curricula, physics-hint text, or the question stem itself, rather than from watching the scene.
That is the value proposition beyond prior benchmarks.

Our contributions are:
\begin{itemize}

\item \textbf{Attributed physics substrate:} MuJoCo videos with per-object, per-frame taxonomy labels, real-valued attributes (mass, friction, elasticity, velocity), and an open generator for reproducible scenes.

\item \textbf{Factor-disjoint splits and graded QA:} Train/val/test regions of the scene-parameter space are deliberately disjoint; questions are tagged by intended reasoning depth for curriculum-style analysis.

\item \textbf{Curriculum and cue diagnostic:} Zero-shot and LoRA studies on three open VLMs under four difficulty curricula and two prompting settings (with/without textual physics hints), interpreted as adaptation and cue sensitivity on PHLOP---with explicit disclosure of text-solvable templates rather than a claim of grounded understanding.

\end{itemize}

To support scrutiny and reuse, we will publicly release the scene generator, the question-generation templates (including the per-template validity flags of Section~\ref{sec:threats}), the rendered videos and taxonomy annotations, and the LoRA fine-tuning and evaluation code under a permissive license, with a versioned dataset card documenting known leakage and the answerable/unanswerable split.\footnote{Repository and dataset links are withheld for anonymous review and will be included in the camera-ready version.}
...
PHLOP builds on these advances while emphasizing denser physical--semantic alignment than most prior VQA-style physics benchmarks. Table~\ref{tab:phlop_full_comparison} summarizes the comparison under the capability definitions in the caption; we mark counterfactual support as partial because PHLOP currently uses verbal what-if prompts rather than engine re-simulation as in ComPhy~\cite{chen2022comphy}.
...
PHLOP particularly emphasizes:

\begin{itemize}
    \item \textbf{Per-frame semantic labeling} of object states and transitions under a formal taxonomy.
    ...
    \item \textbf{Distribution-based material sampling} across density, elasticity, and friction, enabling property-sensitive questions (some of which still admit textual shortcuts; Section~\ref{sec:threats}).
    \item \textbf{Moving event onset detection}, allowing frame-accurate identification of motion transitions.
\end{itemize}

These capabilities support questions such as:
\begin{itemize}
    \item ``Which object experienced the greatest kinetic energy loss?''
    \item ``When did the red ball stop rolling?''
\end{itemize}

By combining continuous dynamics with interpretable labels, PHLOP offers a scalable diagnostic resource for probing how vision--language models adapt to physics-grounded video QA.


\section{Methodology}
In this section, we describe the creation of PHLOP, which involves two core components: (1) the dataset creation process for generating physics-accurate videos with ground-truth annotations, and (2) the diagnostic evaluation framework for probing video--language models on physics-grounded video QA.
...
Unlike symbolic QA in CLEVRER~\cite{yi2020clevrer}, PHLOP's questions are grounded in both semantic labels and physical data. Correct answers are derived from simulation logs, so scoring is automatic and reproducible. Not every template equally requires visual dynamics; we discuss video-optional and answer-leaking types in Section~\ref{sec:threats}.
...
We report accuracy exactly as produced by the official PHLOP evaluator on the held-out \textsc{test} split. For every model we compare five conditions---the non-fine-tuned \emph{zero-shot} baseline plus the four LoRA curricula of Table~\ref{tab:ftconfig} (\textit{easy}, \textit{easy+medium}, \textit{hard}, and \textit{full}/all-tasks)---each under the two prompting settings of Section~\ref{sec:prompt-types} (no physics hint / physics-augmented). Beyond a single overall score, the evaluator also reports accuracy separately for each \emph{test-question difficulty} (Easy, Medium, Hard, Very hard). We treat these results as a \textbf{curriculum- and cue-sensitivity diagnostic}: they show how VLMs adapt to PHLOP under LoRA and textual hints, not that high scores imply grounded Newtonian understanding (Section~\ref{sec:threats}).

\paragraph{Static-camera caveat.}
All experiments use the \texttt{static} (fixed-viewpoint) render, so the difficulty ablation is a like-for-like comparison under one camera. A side effect is that a fraction of questions ($\approx 11\%$) refer to an object or event that the fixed viewpoint does not capture---off-screen or behind an occluder---making them effectively unanswerable from the static view. These items remain in the reported overall accuracy; we do not recompute a filtered metric here. Rendering the same scenes with the \texttt{moving} (orbiting) camera would recover many of these cases and is left to future work.

\subsection{Overall Model Performance}

Table~\ref{tab:finetune-ablation} reports overall accuracy for all three models across the five conditions and both prompting settings, and Figure~\ref{fig:overall-heatmap} shows the complete picture broken down by test-question difficulty. Three findings stand out. First, \textbf{zero-shot accuracy is weak}: without fine-tuning, the models score between $17.8\%$ and $32.9\%$ overall---at or near chance for the multiple-choice format---confirming that off-the-shelf VLMs do not solve PHLOP. Second, \textbf{LoRA fine-tuning raises accuracy substantially}: the best configuration reaches $91.2\%$ for InternVL3, $88.3\%$ for SmolVLM, and $64.0\%$ for the more brittle Qwen2-VL. Third, the \textbf{best curriculum is model-dependent}: SmolVLM peaks under the all-tasks (\textit{full}) curriculum, whereas InternVL3 and Qwen2-VL peak marginally under \textit{easy+medium} ($92.0\%$ and $68.2\%$ with hints), with \textit{full} a close second. The difficulty breakdown in Figure~\ref{fig:overall-heatmap} also reveals a pattern that cautions against over-interpreting ceiling scores: after fine-tuning, the \emph{Hard} and \emph{Very hard} tiers are often answered at near-ceiling accuracy (InternVL3 exceeds $99\%$ on \emph{Hard} and reaches $100\%$ on \emph{Very hard}), while the \emph{Easy} tier lags (e.g.\ $71.7\%$ for InternVL3 without hints). Human-oriented \emph{difficulty} labels (intended reasoning depth) therefore do not track empirical model difficulty. Near-ceiling performance on the hardest-labelled tiers is consistent with \textbf{template learning}---several hard templates can be answered from the question text alone (Section~\ref{sec:threats})---whereas some ``easy'' perception and counting questions remain visually challenging.
...
\begin{figure*}[t]
    ...
    \caption{\textbf{Improvement over each model's own baseline} (percentage points; green $=$ improvement). Layout as in Figure~\ref{fig:overall-heatmap}. Almost every fine-tuned cell improves; large Hard/Very-hard gains should be interpreted with the template-learning caveat in Section~\ref{sec:threats}.}
...
PHLOP currently supports rigid-body dynamics with detailed rule-based annotations. Future extensions should explore deformable bodies, fluid interactions, and differentiable physics engines, broadening the benchmark's scope and realism~\cite{tung2023physion++}.
Broader benchmarks such as PhysicsRW~\cite{physicsrw2024} and PhysReason~\cite{physreason2024} point toward evaluating reasoning under novelty and multi-step causal chains; integrating PHLOP with robotic manipulation or real-world scenes remains future work.
Open-ended generation and learned classifiers for semantic annotation could further improve scalability.

\subsection{Threats to Validity}
\label{sec:threats}

We state these threats explicitly so that the reported accuracies are not over-read as evidence of grounded physical understanding.

\begin{itemize}
    \item \textbf{Answer-leaking and video-optional templates.}
    Several automatically generated types can be solved with little or no visual dynamics: e.g., \texttt{momentum\_conservation} embeds the momentum ratio in the question stem; \texttt{newtons\_second\_law} is a textbook prompt independent of the video; \texttt{property\_competition} pastes numeric mass and friction into the stem. Near-ceiling accuracy on Hard/Very-hard tiers after fine-tuning is therefore consistent with template learning. A text-only (no-video) ablation and a leak-filtered evaluation are left to future work and are \emph{not} claimed here.

    \item \textbf{Verbal counterfactuals, not re-simulation.}
    PHLOP's ``counterfactual'' items are formula- or direction-of-change prompts (e.g., friction/velocity scaling), not engine interventions that remove an object and re-simulate as in ComPhy~\cite{chen2022comphy}.

    \item \textbf{Unanswerable static-view questions.}
    Approximately $11\%$ of test questions refer to off-screen or occluded events under the static camera and remain in the reported overall accuracy (Section~\ref{sec:finetune-results}).

    \item \textbf{Single-seed LoRA and limited model pool.}
    Each configuration is trained once with parameter-efficient adaptation on three small open VLMs; we report point estimates without seed variance, and we do not evaluate closed SOTA video--language models or a human baseline.

    \item \textbf{Confounded OOD split.}
    Train and test differ simultaneously in shapes, materials, object counts, cameras, collision modes, and floors, so failures cannot be attributed to a single factor. Factor-wise leave-one-out ablations are future work.

    \item \textbf{Unquantified annotation QA.}
    Taxonomy labels are rule-based from simulation logs; manual review covered only a sampled subset without reported inter-annotator agreement or error rates.

    \item \textbf{Question imbalance.}
    The $25{,}436$-question \textsc{test} split spans $26$ templates but is heavily skewed: a degenerate \texttt{unknown} bucket accounts for $2{,}801$ items ($11.0\%$; $22{,}635$ remain after excluding it), and among those remaining templates the five most frequent still cover $43.6\%$ of questions. The long tail is correspondingly sparse---templates such as \texttt{event\_sequence} and \texttt{property\_competition} contribute fewer than $60$ items each ($<0.3\%$). Because difficulty tiers are also unevenly populated, a micro-averaged overall accuracy can be dominated by a handful of high-frequency, low-depth categories; we therefore recommend reading the per-template breakdown alongside the aggregate.
\end{itemize}

Addressing these threats---text-only and leak-filtered evals, human baselines, multi-seed runs, factor-wise OOD probes, and rebuilt hard-tier questions that require video evidence---is the natural next step for strengthening PHLOP as a physics-reasoning benchmark.
