...
# with open("questions.txt", "wr") as f:
...
output_path = 'output_with_qa.mp4'
...
for question, answer in sample_qa:
    ...
    background_rect = cv2.addWeighted(background_rect, background_alpha, np.zeros((text_height, width, 3), dtype=np.uint8), 1 - background_alpha, 0)

    y0_start = y0 - 10 
    frame_text[y0_start:y0_start+text_height, 0:width] = cv2.addWeighted(frame_text[y0_start:y0_start+text_height, 0:width], 1 - background_alpha, background_rect, background_alpha, 0)
