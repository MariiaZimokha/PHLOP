...
def generate_constant_velocity_objects():
    ...
    return [
        generate_obj_json_like(
            "cube", "glass", "sliding", [2.0, 2.0, 2.0], [0.0, 0.0, 0.0], 0.9, "0.0"
        ),
        generate_obj_json_like(
            "ball",
            "glass",
...
def generate_stationary_objects():
    return [
        generate_obj_json_like(
            "cube", "glass", "stationary", [0, 0, 0], [0, 0, 0], 0.1, 0.6, 3.0, 0
        ),
        generate_obj_json_like(
            "sphere", "glass", "stationary", [0, 0, 0], [0, 0, 0], 0.05, 0.4, 3.0, 2
        ),
        generate_obj_json_like(
            "cylinder", "metal", "stationary", [0, 0, 0], [0, 0, 0], 0.2, 0.5, 0, 0
        ),
    ]
...
def generate_pure_rotation_objects():
    return [
        ...
        generate_obj_json_like(
            "ball", "wood", "sliding", [0, 0, 0], [0, 0, 10], 0.8, 0.0, -0.5261, 0.8233
        ),
    ]
...
def generate_inelastic_collision_objects():
    return [
        generate_obj_json_like(
            ...
            material="plastic",
            mode="collision",
            velocity=[0.16871682536676982, 1.2085505065767836, 0.01039912189402277],
            angular_velocity=[
                2.4002006700429055,
                -4.408637078310188,
                3.322123085408281,
        ...
        generate_obj_json_like(
            ...
            material="rubber",
            mode="collision",
            velocity=[1.1757714900812308, -0.021501278922900633, -0.08831802324698271],
            angular_velocity=[
                2.687100087471629,
                -1.5428969058136266,
                -3.6053503279078503,
...
def generate_elastic_collision_objects():
    return [
        generate_obj_json_like(
            ...
            material="glass",
            mode="collision",
            velocity=[-2.715869033823274, -2.773728290865599, 0.1953383072039434],
            angular_velocity=[
                0.5614961036775568,
                0.4810220863413195,
                1.0239316560435674,
...
def generate_moving_to_stopping_objects():
    return [
        generate_obj_json_like(
            "sphere", "rubber", "sliding", [6.0, 0, 0], [0, 0, 0], 0.4, 0.85
        ),
        generate_obj_json_like(
            "cube", "cork", "sliding", [5.5, 0, 0], [0, 0, 0], 0.3, 0.75
        ),
    ]
